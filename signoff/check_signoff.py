...
PDK-free -- the checks that need neither `klt` nor a network run in the PR lint
job (`.github/scripts/lint.sh`), and `--run-klt` adds the re-grade for the CI
job that installs the pinned `klt` (`.github/workflows/ci.yml`).
