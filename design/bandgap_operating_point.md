...
| M1, M2 (amp input pair) | `nfet_03v3` | W=200 µm, L=4 µm, nf=8 | **Final (#42), nf corrected (#65).** 2x #10's gate area (20x #8's). L unchanged from the #4 MOS-mismatch characterization geometry (`20260731-031718-8fb0ea6`) so `A_pair` applies with no L-extrapolation; `nf=8` (25 µm/finger, up from the `nf=4` #42 originally declared — #65 found the schematic had never been updated to the 8-finger common-centroid layout `layout/bandgap_top/plan.py` always drew) keeps each finger comfortably inside the *same* 100 µm width bin #10's 100 µm/nf=2 device used, so no bin extrapolation is introduced either. Still the largest single term in the offset budget — `bandgap_error_budget.md` Sec 2.2, 2.7 |
